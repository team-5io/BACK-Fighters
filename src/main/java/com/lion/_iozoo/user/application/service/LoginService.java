...
public class LoginService implements LoginUseCase {
    ...
    public LoginResult login(LoginCommand command) {
        log.info("event=user_login_시작 email={}", command.email());
        ...
        try {
            ...
            log.info("event=user_login_완료 email={}, userId={}", command.email(), user.getId());
            return new LoginResult(user, accessToken);
        } catch (RuntimeException e) {
            log.warn("event=user_login_실패 email={}, reason={}", command.email(), e.getMessage(), e);
            throw e;
        }
    }
}
